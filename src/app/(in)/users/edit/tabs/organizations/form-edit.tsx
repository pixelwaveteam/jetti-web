...
export function UserOrganizationsFormEdit({
  user: { id: userId, ...user },
}: UserFormEditProps) {
    ...
    () =>
      user.organizations.reduce((acc, userOrganization) => {
        const organization = organizations.find(
          ({ id }) => id === userOrganization.organizationId
        );
      ...
      <Form {...formMethods}>
        <form className='mt-4 space-y-6'>
          {orgs.map(({ id, organizationName }, index) => (
            <Popover
              key={id}
              onOpenChange={() => setOrganizationsQuery(undefined)}
            >
                ...
                <FormItem>
                  <FormControl>
                    <div className='relative'>
                      ...
                      {organizationName ? (
                        <button
                          onClick={() => handleDeleteUserOrganization(id)}
                          type='button'
                        ...
                        <button
                          onClick={() => handleNewUserOrganizationDelete(index)}
                          type='button'
                          className='absolute right-2 top-1/2 -translate-y-1/2 p-1 rounded-sm hover:bg-black/5 transition-all'
                        >
                          <Trash
                ...
                {filteredOrganizations.length > 0 ? (
                  ...
                  <p className='text-muted-foreground text-center text-sm'>
                    {organizationsQuery
                      ...
                      : 'Todas organizações foram escolhidas.'}
                  </p>
                )}
